...
namespace RealEstate.UI.Controllers
{
    ...
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int messageId)
        {
            ...
            var responseMessage = await client.DeleteAsync($"Messages/{messageId}");

            if (responseMessage.IsSuccessStatusCode)
            {
                // Silme işlemi başarılı olduysa, kullanıcıyı inbox sayfasına yönlendir
                return RedirectToAction("InBox");
            }
            else
            {
                // Silme işlemi başarısız olduysa, hata mesajını ViewBag ile iletebilir veya uygun bir sayfaya yönlendirebilirsiniz
                ViewBag.errorMessage = "Mesaj silinirken bir hata oluştu. Lütfen tekrar deneyin.";
                return RedirectToAction("InBox"); // veya başka bir sayfaya yönlendirme yapabilirsiniz
            }
        }
